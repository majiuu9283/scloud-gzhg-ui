### 地图
```text
    地图共两套代码，一套互联网地图，一套公安网地图
    互联网
        设置考勤地点页面: /src/views/attend/index.vue
        备注:互联网地图使用需要对应地图的key,在对应地图的开放平台中申请开发者账号,项目中目前使用了高德地图
        高德地图开放平台:https://lbs.amap.com/
    公安网
        设置考勤地点页面: /src/views/attend/index_inner.vue
```
### 自动登录
```text
大屏首页的人员动态按钮通过当前项目的/src/views/vrify/index.vue自动登录,若admin密码修改了，请同步修改该文件内的密码，否则无法自动登录)
```
<p align="center">
...
提交时错误问题解决方法：
git commit --no-verify -m "cmt"   即跳过检查
git push